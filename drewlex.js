  ...
  var featuresFilesSrc = {
    ...
    function genLocalForm () {
        ...
        var localForms = {
            ...
            'rs' : '<div class="order_block"> <div class="prod_img_wrapper"> <img src="' + settings.form.img + '" width="300" class="prod_img">' + ((settings.form.price) ? '<div class="priceс"><span class="discountс">Попуст!</span> <s><span class="js_old_price price_old">' + settings.priceMain * 2 + settings.currencyMain + '</span></s><br> <b class="new_price js_new_price price_main">' + settings.priceMain + settings.currencyMain + '</b> </div>' : '') + '</div> <center> <h3 style="text-align: center">Пожурите да наручите пре него што попуст буде прослеђен следећем читаоцу, <br>ваша јединствена понуда завршава за:  <span class="time_remains" id="min"></span> : <span class="time_remains" id="sec"></span> </h3> </center> <form class="order_form" action="'+settings.form.orderScript+'" method="POST"> ' + (!!settings.form.subs?settings.form.subs:'')  + ' <label for="name" class="data_label">Име и презиме:</label><input class="wheel_input" type="text" name="name" autocomplete="name" required="" id="name" placeholder="Име и презиме"><label for="phone" class="data_label">Телефон:</label> <input class="wheel_input" type="tel" id="phone" name="phone" ' + ((!!settings.form && settings.form.phoneMinLength)?'minlength="'+settings.form.phoneMinLength+'"':'')+' autocomplete="tel" required="" placeholder="+381 123456789"><button class="main-link" type="submit">Наручите ' + settings.product + ' са 50% попуста!</button> </form> </div><center> <h2 class="time_remains_title">Понуда важи пре:  <span class="time_remains">' + outputDat(0, false) + '</span></h2> <br> </center>',
